...
{
	DeleteObject((HGDIOBJ*)&_afxHalftoneBrush);
}
...
void CRectTracker::Draw(HDC pDC) const
{
	...
	if ((m_nStyle & (dottedLine | solidLine)) != 0)
	{
		if (m_nStyle & dottedLine)
			pOldPen = (HPEN)SelectObject(pDC, _afxBlackDottedPen);
		else
			pOldPen = (HPEN)SelectObject(pDC, GetStockObject(BLACK_PEN));
	...
	if ((m_nStyle & hatchInside) != 0)
	{
		...
		nOldROP = SetROP2(pDC, R2_MASKNOTPEN);
		Rectangle(pDC, rect.left + 1, rect.top + 1, rect.right, rect.bottom);
		SetROP2(pDC, nOldROP);
	}
	...
	if ((m_nStyle & hatchedBorder) != 0)
	{
		...
		PatBlt(pDC, rectTrue.left, rectTrue.top, rectTrue.Width(),
			rect.top - rectTrue.top, 0x000F0001 /* Pn */);
		PatBlt(pDC, rectTrue.left, rect.bottom,
			rectTrue.Width(), rectTrue.bottom - rect.bottom, 0x000F0001 /* Pn */);
		PatBlt(pDC, rectTrue.left, rect.top, rect.left - rectTrue.left,
			rect.Height(), 0x000F0001 /* Pn */);
		PatBlt(pDC, rect.right, rect.top, rectTrue.right - rect.right,
			rect.Height(), 0x000F0001 /* Pn */);
	}
...
BOOL CRectTracker::SetCursor(HWND pWnd, UINT nHitTest) const
{
...
BOOL CRectTracker::Track(HWND pWnd, CPoint point, BOOL bAllowInvert,
	HWND pWndClipTo)
{
...
BOOL CRectTracker::TrackRubberBand(HWND pWnd, CPoint point, BOOL bAllowInvert)
{
...
BOOL CRectTracker::TrackHandle(int nHandle, HWND pWnd, CPoint point,
	HWND pWndClipTo)
{
	assert(nHandle >= 0);
